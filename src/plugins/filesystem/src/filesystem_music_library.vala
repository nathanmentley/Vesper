...
namespace Vesper.Plugins.Filesystem {
        ...
        Object
    {
        public PluginKey key {
            owned get
            {
                return new PluginKey("filesystem-1", "Filesystem");
            }
        }
        ...
        public void configure (SettingsEngine settings) {
            metadata_reader = new MetadataReader ();
        ...
        public async Gee.List<Song> get_tracks (string album_id) {
            ...
            try {
                ...
                while ((info = enumerator.next_file ()) != null) {
                    ...
                    // Filesystem-derived fallback title.
                    var title = filename;
                    ...
                    // Embedded metadata overrides the filesystem-derived title.
                    var metadata = metadata_reader.read (song_file);
        
                    if (metadata.title != null &&
                        metadata.title.strip ().length > 0) {
